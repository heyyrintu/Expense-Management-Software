...
- **Reconciliation** (`/bank-recon`, §7.6): the import is a THREE-step sheet and each step ends with a question the reader can answer — did we read your file (row count), do these columns mean what we think (live 5-row preview), is this what will import (a real `stage=validate` dry run on the server, never a re-derivation in the browser). `validateStatementFile` in `lib/schemas/statement-file.ts` is the one function the dropzone and the route both call. The column mapping is saved only when the reader ticks the box — it used to save on every import, quietly making a one-off file from another bank the default for everyone.
- **Recon bucket tones are not interchangeable**: Matched is success; **"Not in bank" is DANGER** (the app says money moved and the bank disagrees) and carries NO action, because no button on that screen can resolve it and offering one would imply otherwise; **"Not in app" is WARNING** (usually a real payment nobody recorded) and carries both ways to explain a debit. The unexplained total is always the danger token, except at zero where it goes green — a red ₹0 on the goal state teaches readers to ignore the colour. The match dialog's candidates and the "Not in bank" bucket come from ONE query, so the dialog can never offer a payment the board doesn't show.
- **A destructive confirmation enumerates its consequences.** `LockDialog` lists exactly what becomes read-only AND what stays possible; "Are you sure?" is not informed consent, and a reader who has to guess a lock's scope will either avoid the feature or discover it by being blocked later.
- **Complaints** (`/complaints`, §7.7): the type picker is four visible CARDS with a line of explanation each, never a dropdown — the type decides who handles the dispute and what evidence attaches, so it must be answerable by reading. Nothing is preselected. The thread is a support conversation, NOT a chat app: no bubbles, no tails, no right-aligned messages; own messages are marked by a quieter avatar and a "You" label, because right-aligning destroys the left edge that makes a long thread readable and this thread is evidence. Thread timestamps are relative (activity context); the disputed payment's date in the header card stays absolute. The thread stays open after the complaint closes — the record is frozen, the conversation isn't. Won't-fix takes a NEUTRAL card, never danger: declining to act is a legitimate outcome, and red reads as an error to chase.
- **Complaints inbox** is finance-only and takes the shared `components/data-table` — it is an ordinary list of records, so it earns no exception. Facets come from `FacetSelect` (key-generic since D4.3) driven by `lib/domain/complaint-filters.ts`, not the `FilterBar` wrapper, which is bound to the expense URL schema. Selecting two age buckets WIDENS (`ageFloorBusinessDays` takes the minimum) — a multi-select that returns fewer rows for a second tick is a bug nobody reports. Breached rows only get the `overdue-edge` inset shadow; edging amber rows too would mark two thirds of a busy inbox and leave no emphasis at all. `rowClassName` on DataTable must never change a row's box, or the table jitters as ages tick over.
- **`AnimatedStatusBadge`** is for the two places a status changes under the reader's eyes (thread header, inbox row): 150ms opacity crossfade via `mode="popLayout"` — `mode="wait"` would queue the exit after the enter and spend 300ms on a badge. Everywhere else uses the plain server-rendered badge.
- **Settings** (`/settings/*` + `/profile`): every screen is `SettingsPanel` inside the two-column shell — left `SettingsNav`, right panel, one column under md. Nav lives in `lib/settings/nav.ts` and its `minRole` MIRRORS each route's own guard (asserted in `tests/unit/settings-nav.test.ts`); hiding a link is never a permission.
- **The settings form pattern is `DirtySaveBar`**: the save bar appears only once the form is dirty against the LAST SAVED values, with Discard beside it. A permanently visible Save is wrong most of the time, and its appearance is the signal that something is unsaved. After a successful save, re-baseline (`form.reset(values)`) so the bar retracts — leaving stale defaults keeps the form dirty against a state that no longer exists.
- **Destructive confirmations use `ConfirmDestructiveDialog`**, which NAMES the exact entity in its title, enumerates what changes and what survives, and never gives the confirm button focus (Cancel is first in the DOM and confirm sets `autoFocus={false}`). The page-level trigger is `secondary`; the danger styling belongs on the confirm inside the dialog, where the reader has actually read the consequences.
- **Masking is a property of what was SENT, not what is displayed.** `MaskedValue`'s reveal calls a server action that takes NO id and resolves identity from the session, so it cannot be aimed at another row; hiding drops the value from state. A bank account number reaches the browser only on request, is never echoed back into an edit field, and finance's view of someone else's details stays presence-only.
- **The WhatsApp OTP flow is a state machine** (`lib/domain/whatsapp-link.ts`), not interacting booleans: idle → sending → code_sent → verifying → linked. A refused NUMBER returns to idle; a wrong CODE returns to code_sent, keeping the number — a typo in six digits must not restart the flow.
- **Every route owes four states** — empty, loading, error, failed-mutation toast. `docs/STATES-AUDIT.md` is the register; add a row when you add a route. Skeletons compose `components/ui/page-skeleton`, whose pieces read the SAME tokens as the real components (`h-row`, `--ledger-viewport-height`, PageHeader's `pb-6`), which is what keeps CLS near zero — a skeleton that merely looks similar drifts the first time a card is resized, and nobody sees both states within the same second.
- **Error boundaries inherit**: `app/(app)/error.tsx` catches every tenant screen, and a route gets its own only when the useful recovery DIFFERS — on a detail screen it does, so those pass `backHref` to `RouteError` and offer both retry and a way back.
- **Failed mutations go through `notify.failed()`**, which distinguishes offline from a server refusal (`lib/errors.ts`); a server-supplied message always wins, because it knows whether the period was locked or the report already paid. Forms report failures INLINE beside the fields via `offlineAwareMessage()` rather than as a toast that can be missed.
- **Copy voice is enforced, not asserted**: `scripts/check-copy-voice.mjs` runs in `npm run lint` and fails on exclamation marks, apologies, "please try again", "something went wrong" and second-person blame in any user-facing `headline`/`description`/`emptyMessage`/`title` prop. `/design-system` is exempt — it quotes bad copy deliberately.
- **Accessibility is checked, not assumed** (`docs/A11Y-AUDIT.md`). `scripts/check-contrast.mjs` runs in `npm run lint` and measures EVERY foreground token against every surface it lands on — a hand-written contrast list is how `--fg-tertiary` shipped at 2.56:1 for five milestones. `tests/e2e/a11y.spec.ts` runs axe over 20 routes plus overlays (`npm run test:a11y`), with `color-contrast` disabled because the script covers it deterministically.
- **A badge is not a live region.** `role="status"` on `StatusBadge`/`SlaBadge` made every row of a table an aria-live region. Status is conveyed by a dot PLUS the word, with an `sr-only` prefix for the category — never by colour, and never by hijacking the announcement queue.
- **Error text uses `text-status-danger-text`**, never the shadcn `text-destructive` compat token, which is unmeasured against these surfaces. Every `<th>` carries `scope="col"`; an actions column with no header still needs an `sr-only` name.
- **Charts load lazily** (`components/charts/lazy.tsx`, `next/dynamic` + `ssr: false`): Recharts is ~130 kB and sits below the KPI strip, and importing it directly made `/dashboard` the heaviest route in the app at 342 kB. The fallback is shape-matched to `ChartFrame`'s own skeleton — a lazy boundary without a matched fallback trades bundle size for a layout shift. `docs/PERF-AUDIT.md` records the numbers.
- **Receipt tiles are never `next/image`**: the URLs are signed, short-lived and private, so the optimizer would miss its cache on every signature rotation and proxy private documents through the app server. They carry `loading="lazy"`, `decoding="async"` and explicit dimensions instead; the real fix is a thumbnail generated at upload time.
- **A paginated list must be provably stable at scale.** `tests/isolation/pagination-scale.test.ts` seeds 200 rows with colliding sort keys and asserts every row appears exactly once across all pages, twice running — the failure `EXPENSE_LIST_ORDER` exists to prevent.
