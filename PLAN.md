...
- [ ] **2.0 User management**: org_admin screen — list users, invite (email + role + department), edit role/department/**assigned approver**, deactivate. Departments CRUD. Prerequisite for 2.2 approval routing.
- [x] **2.0 User management**: org_admin user list (search/filter/pagination), invite with org-bound token + resend/revoke, edit role/department/approver (last-org-admin + self-approver guards), deactivate/reactivate, departments CRUD.
