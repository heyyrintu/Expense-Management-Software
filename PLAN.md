...
- [ ] **1.1 Categories & org settings**: finance_admin CRUD for categories (limits, receipt threshold), org settings (currency, mileage rate). 
- [ ] **1.2 Expense CRUD**: create/edit/delete Draft expenses (amount, date, merchant, category, project, purpose); money as minor units; list + detail views.
...
- [ ] **2.1 Report workflow**: group expenses into report; state machine in `lib/domain/report-workflow.ts` with AuditLog on every transition; withdraw while Submitted.
