...
- [ ] **D5.4 Performance & responsive pass**: Lighthouse ≥90 on dashboard, CLS <0.05, INP <200ms; test at 360px, 768px, 1024px, 1440px; font loading without FOUT; image/receipt lazy loading.
  ...
  - Done 2026-09-03: mobile-preset Performance is **92 on `/dashboard`** (median of 5; was 62), 88 on `/expenses` and 87 on `/expenses/new` (were 72 / 72). Five causes found and fixed — the full framer-motion runtime on every route, zod on the dashboard, Recharts fetched at hydration on phones that could not see a chart, 109 KB of latin-ext fonts pulled in by the rupee sign, and the page header streaming behind the aggregate queries. `docs/PERF-AUDIT.md` §1 has the numbers per change. The "70 KB unminified" of 2026-09-02 was the antivirus on the measuring machine, not the app.
  - **Not done: `/expenses` and `/expenses/new` are still under 90 on the mobile preset.** What remains is fonts (94 KB preloaded) and the fixed React/Next cost; the levers left are design decisions, listed in PERF-AUDIT §1. INP is unmeasured (needs a DevTools interaction trace), and the 360/768/1024/1440 walkthrough is only covered at two widths by the screenshot baseline.
