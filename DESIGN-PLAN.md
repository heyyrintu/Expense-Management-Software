...
- [ ] **D5.3 Accessibility audit**: axe automated pass in CI, keyboard walkthrough of the three critical flows, screen-reader pass, contrast verification, focus-order fixes. Target Lighthouse a11y ≥95.
  ...
  - Done 2026-09-02: the axe suite runs green in CI on every push (first run surfaced one critical finding, fixed); Lighthouse accessibility is **100** on `/login`, `/signup`, `/dashboard`, `/expenses` and `/expenses/new` (target ≥95).
  - **Not done: the keyboard walkthrough of the three flows and the screen-reader pass.** Both are manual; `docs/A11Y-AUDIT.md` gaps 2–3 say where to look first.
- [ ] **D5.4 Performance & responsive pass**: Lighthouse ≥90 on dashboard, CLS <0.05, INP <200ms; test at 360px, 768px, 1024px, 1440px; font loading without FOUT; image/receipt lazy loading.
  ...
  - Done 2026-09-02: Lighthouse run on all three routes (`docs/PERF-AUDIT.md` §1). CLS is 0 everywhere; `/dashboard` scores **98** on the desktop preset.
  - Done 2026-09-03: mobile-preset Performance is **88 on `/dashboard`** (median of 5; was 62), 87 on `/expenses` and 86 on `/expenses/new` (were 72 / 72). Four causes fixed — zod on the dashboard, Recharts fetched at hydration on phones that could not see a chart, 109 KB of latin-ext fonts pulled in by the rupee sign, and the page header streaming behind the aggregate queries. A fifth, framer-motion's full runtime, was fixed with `LazyMotion`, measured at 92, and REVERTED: `m` elements sit at their hidden `initial` style until the feature chunk lands, and the load makes React client-render pending streamed boundaries (the login form briefly existed twice; the e2e sign-in failed). `docs/PERF-AUDIT.md` §1 has the numbers and the reasoning. The "70 KB unminified" of 2026-09-02 was the antivirus on the measuring machine, not the app.
  - **Not done: all three routes sit at 86–88 on the mobile preset, against ≥90.** What remains is fonts (94 KB preloaded), the fixed React/Next cost and the 39 KB of framer-motion kept on purpose; the levers left are design decisions, listed in PERF-AUDIT §1. INP is unmeasured (needs a DevTools interaction trace), and the 360/768/1024/1440 walkthrough is only covered at two widths by the screenshot baseline.
- [x] **D5.5 Design QA**: walk `/design-system` gallery, verify token-only styling (lint clean), consistent status colors across table/card/chart/print, and cross-screen consistency of amounts, dates, and empty states.
  ...
  - Done 2026-09-01: the screenshot baseline is captured and committed — 14 screens × 2 widths in `docs/screenshots/`, indexed by its README.
