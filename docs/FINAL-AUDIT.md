...
| **G7** | — | **The migration history could not be replayed on an empty database.** `20260819044600_dep` sorted 7th but altered `complaints` (created 15th) and dropped an index created 17th — `P3006`/`P1014` on any fresh database. It survived 17 commits because the only database it ever ran against already had the objects. Two silent consequences beyond the replay failure: the dev database was **drifted** from `schema.prisma` in two ways (complaint FKs left `RESTRICT` instead of `SET NULL`; a WhatsApp index left in place that the schema does not declare). Fixed 2026-08-23 by renaming the folder to `20260821190000_complaint_fk_and_whatsapp_index` — SQL untouched. | **Critical** |
| **G8** | — | **CI has been red since 2026-08-20, then stopped running.** The bad migration reached `origin/master` in the push at 06:29 UTC that day; `migrate deploy` against the fresh postgres service would have failed on it and on all 8 pushed commits after. Since then nothing has been pushed — 9 local commits, including every commit of the last five sessions, have never reached origin, so CI has not run on any of them. The workflow trigger itself is correct (`push` to `master`, and `master` is the branch). | **High** |
...
12. ~~Monthly summary as PDF attachment~~ — **done 2026-08-23**. The CSV no longer sits in the message body; a one-page PDF and the CSV are both real attachments. PDF is generated by a dependency-free writer (`lib/exports/pdf.ts`) — a headless browser to render six numbers would put ~300 MB of Chromium in a cron route, and a PDF library is a runtime dependency for a document that is six numbers and a short table. That writer is text-only and single-page **on purpose**; anything needing a chart should take a real library instead of growing it. 21 unit assertions, including that the xref byte offsets actually resolve.
13. ~~Sync DESIGN-PRD §5 token names~~ — **done 2026-08-23**. §5.1 and §5.2 rewritten to the implemented names, with the Tailwind utility each maps to, the three values the D5.3 contrast pass corrected, and why accent and status each carry two shades. §5.2 now lists all 22 STATUS_MAP keys by their database value.
14. ~~Delete dead dark-theme block~~ — **done 2026-08-23**. Deleted (the shadcn block went earlier, in the token-integrity commit). Decision recorded in DESIGN-PRD §3. `scripts/check-contrast.mjs` also had its `:root` parser re-bounded by brace depth: it used to slice up to the literal string "/* Dark theme hook", so deleting that comment would silently have swept the whole stylesheet into the token set.
15. ~~Make FX stub bidirectional~~ — **done 2026-08-23**. The table is now a single column of X → base quotes and every pair is derived: direct, inverted, or crossed through the base. All 64 supported pairs resolve in both directions, and each returned rate is asserted to be one `isValidFxRate` accepts — a prefill the form then rejects is worse than no prefill. Still a stub; manual override unchanged.
16. ~~Remove or wire `check-rls-state.mjs`~~ — **done 2026-08-23**. Deleted. It could not join `npm run lint` (lint is static and runs before `migrate` in CI), and its coverage half duplicated `tests/isolation/rls.test.ts` against a hard-coded list of five tables rather than every table with an `org_id`. Its one unique value — diagnosing a SUPERUSER connection, which makes every RLS assertion pass vacuously — is now a precondition test at the top of that suite, so the diagnosis survives and the third source of truth does not.
